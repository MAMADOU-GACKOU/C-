    ...
    , ui(new Ui::FenetrePrincipale)
{
    ...
    /*!
     *\brief convertir json pour un contact .
     *connection lié à la conversion json d'un contact
     */
    connect(ui->Bjson,SIGNAL(clicked()),this,SLOT(JsonContact()));
    /*!
     *\brief convertir json pour une intéraction  .
     *connection lié à la conversion json d'un contact
     */
    connect(ui->BJsonInteraction,SIGNAL(clicked()),this,SLOT(JsonInteraction()));
...
/****création et ajout des widget***********/
    ...
    connect(combo,SIGNAL(currentTextChanged(const QString  & )),this,SLOT(Remplir(const QString  & )));/*!< la connection de la combo au IHM lorsque l'on appui sur la Combo box */
...
void FenetrePrincipale:: Remplir(const QString  & valeur)
{
   std::list<Contact> lp;
   lp.clear();
   emit sigGetContactList(lp);
   ...
   for(auto &p:lp)
   {
        if(p.getId_contact()==val)
        {
            n1->setText(QString::fromStdString(p.getNom()));
            p1->setText(QString::fromStdString(p.getPrenom()));
            en1->setText(QString::fromStdString(p.getEntreprise()));
            em1->setText(QString::fromStdString(p.getMail()));
             ph1->setText(QString::fromStdString(p.getURIPhoto()));
            tel1->setText(QString::fromStdString(p.getTelephone()));
...
void FenetrePrincipale::SupprimerContact()
{
    ...
    connect(comboBox,SIGNAL(currentTextChanged(const QString  & )),this,SLOT(RemplirSup(const QString  & )));/*!< la connection de la combo au IHM lorsque l'on appui sur la Combo box */
...
void FenetrePrincipale::RemplirSup(const QString  & valeur )
{
    std::list<Contact> lp;
    lp.clear();
    emit sigGetContactList(lp);
...
void FenetrePrincipale::RemplirSupInter(const QString  & valeur )
{
    std::list<Contact> lp;
    lp.clear();
    emit sigGetContactList(lp);
...
void FenetrePrincipale::ListeInteraction()
{
    ...
    intertexts=new QLabel("nom/Prenom");/*!< décalration du QLabel pour le contact à liste d'interaction */
     interactiontexts=new QLineEdit();/*!< décalration du QLabel pour le contact à son nom */
    ...
    connect(comboList,SIGNAL(currentTextChanged(const QString  & )),this,SLOT(RemplirSupInters(const QString  & )));/*!< la connection de la combo au IHM lorsque l'on appui sur la Combo box */
...
void FenetrePrincipale::RemplirSupInters(const QString  & valeur )
{
    std::list<Contact> lp;
    lp.clear();
    emit sigGetContactList(lp);
...
void FenetrePrincipale::RechercheInteractionContact()
{
   ...
   interactiontextsc=new QLineEdit();/*!< décalration du QLabel pour le contact à son nom */
    ...
    connect(comboRecherche,SIGNAL(currentTextChanged(const QString  & )),this,SLOT(RemplirSupIntersc(const QString  & )));/*!< la connection de la combo au IHM lorsque l'on appui sur la Combo box */
...
void FenetrePrincipale::RemplirSupIntersc(const QString  & valeur )
{
    std::list<Contact> lp;
    lp.clear();
    emit sigGetContactList(lp);
...
/*!
 *\fn FenetrePrincipale::JsonContact()
 *\brief Json export
 *la fonction qui nous permet d'exporter tous les contacts sur un fichier JSon
 */
void FenetrePrincipale::JsonContact()
{
    std::list<Contact> lp;//Déclaration d'une liste
    bd.ListeContact(lp);//on récupère la liste de contact dans la base de donnée .
   ...
   for(auto &p:lp)
    {
         ...
         obj.insert("Photo ",QString::fromStdString(p.getURIPhoto()));
 ...
 *la fonction qui nous permet d'exporter tous les contacts sur un fichier JSon
 */
void FenetrePrincipale::JsonInteraction()
{
    std::list<Interaction> inter;//Déclaration d'une liste
    inBDD.ListeInteractionALL(inter);//on récupère la liste de contact dans la base de donnée .
    QJsonArray table;//on crée une table
...
if(ret==QMessageBox::Ok)
 {
   for(auto &p:inter)
    ...
    QFile file("/tmp/Interaction.json");
    if(!file.open(QIODevice::ReadWrite | QIODevice::Text ))
       return;
    QTextStream out(&file);
    out.setCodec("UTF-8");
